<template>
  ...
  export default {
    ...
    methods: {
      ...
      startSlideshow() {
        this.intervalId = setInterval(this.nextBeer, 6000); // Automatically switch images every 6 seconds
      },
    },
    mounted() {
      this.startSlideshow();
  ...
  .beer-slideshow {
    ...
    height: 40rem;
    overflow: hidden;
    animation: fadeAnimation 6s ease-in-out infinite; /* Apply animation to the whole slideshow */
  }
  ...
  .fade-image {
    animation: fadeAnimation 5s linear infinite; /* Apply fade animation with the same speed as breweries */
